...
def check_correction_alert(rows):
    alerts = []
    for row in rows:
        hist_df = row.get("history")
        if hist_df is not None and not hist_df.empty:
            hist_df = hist_df.copy()
            hist_df = hist_df.set_index(hist_df.columns[0])
            ind = compute_indicators(hist_df)
            rsi = ind.get("rsi14_last")
            macd = ind.get("macd_last")
            signal = ind.get("signal_last")
            if rsi is not None and macd is not None and signal is not None:
                if rsi > 70 and macd < signal:
                    alerts.append((row["symbol"], rsi, macd, signal))
    return alerts
